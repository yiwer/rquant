...
def run(cfg, axis, top, label, reb=1, sectors=None, membership=None):
    """Tier-1：gross(0) + net(COST)。reb=1 且无 sectors/membership 复用 run_once；否则走 _screen（部署节奏/行业中性/点时成分）。"""
    a = AXES[axis]
    ...
    if reb == 1 and not sectors and not membership:
        ...
        g = _screen(cfg, 0.0, a, top, reb, g_out, uni, sectors, membership)
        n = _screen(cfg, COST, a, top, reb, n_out, uni, sectors, membership)
    return g, n, a


def _screen(cfg, cost, a, top, reb, out, uni, sectors=None, membership=None):
    """直接调引擎（支持 --rebalance 扫描 + --sectors 行业中性 + --membership 点时成分=去幸存者偏差，run_once 不支持故不可复用）。"""
    import subprocess
...
def tier2_sweep(cfg, axis, label, bench=None, sectors=None, membership=None):
    """敏感性：top∈{30,50,100} × reb∈{1,5} 各跑 net，收集净超额（检符号翻转）。
    bench 给定时超额改 vs 指数（与主裁决同口径）；sectors 给定时行业中性(per_sector 来自配置)；membership 给定时点时成分(同主裁决口径)。"""
    a = AXES[axis]
    uni = os.path.join(de.REPO_ROOT, a["universe"])
    ...
    for top in (30, 50, 100):
        for reb in (1, 5):
            ...
            rep = _screen(cfg, COST, a, top, reb, o, uni, sectors, membership)
            if idx:
...
def main():
    ...
    ap.add_argument("--membership", default=None, help="点时成分 CSV(date,symbol)：每调仓仅在当日成分内选股(去幸存者偏差)")
    a = ap.parse_args()
    ...
    membership = os.path.join(de.REPO_ROOT, a.membership) if (a.membership and not os.path.isabs(a.membership)) else a.membership
    rnd = _next_round()
    prior = _prior_best_oos()
    g, n, ax = run(a.config, a.axis, a.top, label, a.rebalance, sectors, membership)
    bench = a.benchmark
    ew = None
    ...
    if v0 == "PASS":                                    # Tier-2 仅过 OOS 门触发
        sweep = tier2_sweep(a.config, a.axis, label, bench, sectors, membership)
